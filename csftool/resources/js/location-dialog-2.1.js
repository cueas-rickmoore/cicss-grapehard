...
var MapDialog = {
    ...
    geocodeAddress: function(dataArea) {
        ...
        var address = jQuery('#over_map').find('input[id="csftool-address-form"]').val();
        var callback =
            (function(loc) { var loc = loc; return function(results, status) {
                    if (status === MapDialog.google.maps.GeocoderStatus.OK && results.length > 0) {
                        ...
                        jQuery('#over_map').find('input[id="csftool-address-form"]').val("");
    ...
    initializeDialog: function(dom_element) {
        ...
        var address_form = ['<div id="over_map">',
                            ...
                            '<input type="text" id="csftool-address-form" placeholder="Enter County, State, Zip Code, or Address">',
                            '</form>',
                            '</div>'].join('');
        jQuery(".csf-map-dialog").children(".ui-dialog-titlebar").children(".ui-dialog-title").append(address_form);
...
var MapDialogOptions = { appendTo: DialogDOM.map_anchor, autoOpen:false,
    ...
    //bnb have to turn drag ability off in order to focus form in titlebar
    //draggable: true,
    draggable: false,
    minHeight: 400,
    minWidth: 400,
