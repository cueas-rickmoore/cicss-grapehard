...
var DialogDOM = {
    dialog_anchor:"#csftool-location-dialog-anchor", 
    dialog_container:"#csftool-location-dialog",
    dialog_html: [ '<div id="csftool-location-dialog">',
                   '<div id="csftool-location-dialog-anchor">',
                   '<div id="csftool-location-dialog-content">',
                   '<div id="csftool-location-dialog-data">',
                   ...
                   '</div> <!-- close csftool-location-dialog-data -->',
                   '<div id="csftool-location-dialog-map" class="map-container"> </div>',
    ...
    map_anchor:"#csftool-location-dialog-map",
}

var DialogController = {
    
    close: function() {
        jQuery(DialogDOM.dialog_anchor).dialog("close");
        this.execCallback("close", jQuery.extend({}, DialogContext.selected_location));
    },

    deleteLocation: function() {
        var ld = DialogDOM.location_dom;
        var _location = { name:jQuery(ld.name).val(), description:jQuery(ld.description).val(),
                          lat:jQuery(ld.lat).val(), lng:jQuery(ld.lng).val() };
    ...
    dialogButtons: function() {
        return { Cancel: { "class": "csftool-location-dialog-cancel", text:"Cancel",
                 ...
                 Done: { "class": "csftool-location-dialog-done", text:"Done",
                         click: function () {
    ...
    initializeDialog: function(dom_element) {
        ...
        context.container = dom_element;
        console.log("dialog html installed in : " + context.container);
        var buttons = jQuery.extend({}, this.dialogButtons());
        console.log("creating modal dialog in : " + dom.dialog_container); 
        jQuery(dom.dialog_anchor).dialog( {
            autoOpen:false,
            appendTo: dom.dialog_container,
            modal: true, buttons: buttons, title: context.title,
            minHeight:context.dimensions.height, minWidth:context.dimensions.width,
            position: { my: "center center", at: "center center", of: dom.center_on },
            } );
...
var MapController = {
    ...
    initMap: function(options) {
        ...
        MapContext.map = new google.maps.Map(document.getElementById(MapContext.dom_element), MapOptions);
