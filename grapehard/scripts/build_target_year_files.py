...
parser.add_option('-d', action='store_true', dest='dev_build',
                  default=False)
...
debug = options.debug
dev_build = options.dev_build
verbose = options.verbose

if dev_build:
    factory = GrapeHardinessBuildFactory('dev')
...
data = reader.timeSlice('reported.maxt', season_start, season_end)
builder.open('a')
...
data = reader.timeSlice('reported.mint', season_start, season_end)
builder.open('a')
...
for variety_key in project.varieties:
    ...
    reader = \
        factory.buildFileReader('build', target_year, source, region, variety)
    # build a new file
    builder = factory.varietyFileBuilder(variety, target_year, source, region,
                                         'season', bbox=reader.data_bbox)
    print '    building file :', builder.filepath
    print '    from data in :', reader.filepath
    ...
    print '    building hardtemp dataset'
    ...
    builder.open('a')
    builder.buildDataset('hardtemp', data=data)
    builder.close()
